...
try {
    ...
    // Insert the book IDs into the book_ownership table with purchase date and time
    ...
    $purchase_date = date('Y-m-d'); // Format: YYYY-MM-DD
    $purchase_time = date('H:i:s'); // Format: HH:MM:SS
    ...
    while ($row = $selectBooksStmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $insertOwnershipStmt->bindParam(':purchase_date', $purchase_date, PDO::PARAM_STR);
        $insertOwnershipStmt->bindParam(':purchase_time', $purchase_time, PDO::PARAM_STR);
